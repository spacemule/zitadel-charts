...
| zitadel.configFilePaths | list | `[]` | Paths to files mounted into the container via an extraVolumeMount. Use this to merge sensitive ZITADEL configuration values from sources such as the secrets-storage-csi driver without exposing the values to etcd and other processes. |
...
| zitadel.masterkey | string | `""` | ZITADEL's masterkey for symmetric encryption of sensitive data like private keys and tokens. Must be exactly 32 bytes. Using printable ASCII characters is recommended (alphanumeric). Do NOT use multi-byte Unicode characters, as the key length is measured in bytes, not characters. Generate with: tr -dc A-Za-z0-9 </dev/urandom | head -c 32 IMPORTANT: Store this value securely. Loss of the masterkey means loss of all encrypted data. Either set this value, masterkeySecretName, or masterkeyFilePath. |
...
| zitadel.masterkeyFilePath | string | `""` | Path at which the masterkey file is mounted into the container. This allows the use of the secrets-storage-csi driver or similar solutions to manage the secret. This option requires configuring extraVolumes and extraVolumeMounts. Note: One of zitadel.masterkey, zitadel.masterkeySecretName, or zitadel.masterkeyFilePath must be set. |
| zitadel.masterkeySecretName | string | `""` | Name of an existing Kubernetes Secret containing the masterkey at key "masterkey". Use this for production deployments to avoid storing the masterkey in values files. The secret must exist before chart installation. Note: One of zitadel.masterkey, zitadel.masterkeySecretName, or zitadel.masterkeyFilePath must be set. |
